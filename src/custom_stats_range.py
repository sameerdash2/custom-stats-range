...
def _done_NEW(self, end, chunk: int = 1, start = 0):
    ...
    if end is not None:
        lims.append(
            "id > %d" % ((get_day_cutoff(self.col.sched) - (end * chunk * 86400)) * 1000)
        )
        # CSR line
        lims.append(
            "id < %d" % ((get_day_cutoff(self.col.sched) - (start * chunk * 86400)) * 1000)
        )
...
def _daysStudied_NEW(self):
    ...
    if end:
        lims.append("id > %d" % ((get_day_cutoff(self.col.sched) - (end * 86400)) * 1000))
        # CSR line
        lims.append("id < %d" % ((get_day_cutoff(self.col.sched) - (start * 86400)) * 1000))
...
def _eases_NEW(self) -> Any:
    ...
    if end is not None:
        lims.append(
            "id > %d" % ((get_day_cutoff(self.col.sched) - (end * 86400)) * 1000)
        )
        # CSR line
        lims.append(
            "id < %d" % ((get_day_cutoff(self.col.sched) - (start * 86400)) * 1000)
        )
    if lims:
        lim = "where " + " and ".join(lims)
    else:
...
def _hourRet_NEW(self):
    ...
    start, end = _periodDays_NEW(self)
    if end:
        lim += " and id > %d" % ((get_day_cutoff(self.col.sched) - (86400 * end)) * 1000)
        # CSR line
        lim += " and id < %d" % ((get_day_cutoff(self.col.sched) - (86400 * start)) * 1000)
    return self.col.db.all(
        f"""
select
...
group by hour having count() > 30 order by hour"""
        % lim,
        get_day_cutoff(self.col.sched) - (rolloverHour * 3600),
    )
